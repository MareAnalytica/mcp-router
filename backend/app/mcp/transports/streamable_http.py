...
class StreamableHTTPTransport(BaseTransport):
    ...
    async def connect(self, url: str, headers: dict[str, str] | None = None, **kwargs: Any) -> ClientSession:
        ...
        logger.info("Streamable HTTP transport connected for server=%s user=%s", self.server_id, self.user_id)
        return session
    ...
    async def disconnect(self) -> None:
        ...
        self._session = None
        logger.info("Streamable HTTP transport disconnected for server=%s user=%s", self.server_id, self.user_id)
